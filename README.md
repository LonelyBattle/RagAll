...
| # | 文件 | 覆盖知识点 |
|---|------|-----------|
| 33 | [`33_evaluation_set.ipynb`](33_evaluation_set.ipynb) | 评测集构造 / 四种来源 / 标注粒度 / 字面命中率 / 零成本基线 / 难负例与拒答样本 / LLM 合成题目 / 版本化与 holdout |
...
│   ├── 星云智能产品手册.md      #   产品/部署/计费/安全    ← 04 05 07 15 33 34 41 主干检索
...
│   ├── 评测集.md                #   人工标注（问题→相关文档/小节），不进检索索引 ← 33 34 35 41
│   ├── 评测集_v2.md             #   第 33 课产出（+合成题+难负例，带类型/来源列），不进检索索引
...
> 除第 33 课产出的评测集与第 34/35/41 课使用的人工标注评测集（`data/评测集*.md`，一律不进检索索引）外，各课的检索、重排、生成都跑在同一套真实语料上：
...
- **动手优先**：改 chunk 大小、top-k、是否混合检索 / 重排，观察指标变化（配合第 33/34/35 课：先造题，再评估）；
